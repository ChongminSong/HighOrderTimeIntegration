...
%% Waitbar
% Be careful when using the waitbar for small size problems as this takes
% some time to update it.
if nDOF > 1e4	% Change this to 0 if you always want the waitbar
    flag_wb = true; 	% Activate the waitbar
else
    flag_wb = false; 	% Deactivate the waitbar
end
...
[T,U,V,A] = PTIM(order,rho_inf,ndt,dt,ft,Kc,Mc,Cc,Fc,U0,V0,saveDOF,flag_wb);
